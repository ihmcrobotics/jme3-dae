...
public class SceneTransformer implements TransformerPack<Tuple2<DAENode, Node>, Void>
{
   ...
   /*
    * Instance controller required because the joints referes to sids in the context of the
   ...
   private void parseNode(DAENode node, Node jmeRoot)
   {
      ...
      TransformedValue<String[]> optionaLayers = node.getAttribute(Names.LAYER, NAME_LIST);
      ...
      List<DAENode> transformElements = node.getChildren(Names.LOOKAT, Names.MATRIX, Names.ROTATE, Names.SCALE, Names.SKEW, Names.TRANSLATE);
      ...
      List<DAENode> optionaInstanceNodes = node.getChildren(Names.INSTANCE_NODE);
      List<DAENode> optionaNodes = node.getChildren(Names.NODE);
   ...
   private void parseInstanceLight(DAENode instanceLight, Node jmeNode)
   {
      ...
      checkTrue(light.isDefined(), "Cannote resolve link " + url.get());
      ...
      checkTrue(techniqueCommon.isDefined(), "Collara 1.4.1 requires a technique_common chid for light element");
