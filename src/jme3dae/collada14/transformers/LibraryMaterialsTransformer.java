...
public class LibraryMaterialsTransformer implements TransformerPack<Tuple2<DAENode, AssetManager>, Void>
{
   ...
   private void parseEffect(DAENode material, DAENode instanceEffect, DAENode effect, AssetManager assetManager)
   {
      ...
      if (!extraList.isEmpty())
      {
         ...
         {
            parseExtra(extra, material.getParsedData(Material.class));
         }
      }
   }
   ...
   private void parsePhong(DAENode material, DAENode instanceEffect, DAENode effect, DAENode phong, AssetManager assetManager)
   {
      ...
      TransformedValue<Texture> reflectiveTexture = specular.getChildValue(Names.TEXTURE, TEXTURE);
...
//    gen.setEmission(emission.getChild(Names.COLOR).getContent(COLOR).get());
//    gen.setEmission(emission.getChildValue(Names.TEXTURE, TEXTURE).get());
//    gen.setReflective(reflective.getChild(Names.COLOR).getContent(COLOR).get());
//    gen.setReflective(reflective.getChildValue(Names.TEXTURE, TEXTURE).get());
//    gen.setReflectivity(reflectivity.getChild(Names.FLOAT).getContent(FLOAT).get());
//    gen.setTransparent(transparent.getChild(Names.COLOR).getContent(COLOR).get());
//    gen.setTransparent(transparent.getChildValue(Names.TEXTURE, TEXTURE).get());
//    gen.setTransparency(transparency.getChild(Names.FLOAT).getContent(FLOAT).get());
//    gen.setIndexOfRefraction(indexOfRefraction.getChild(Names.FLOAT).getContent(FLOAT).get());
   ...
   private void parseExtra(DAENode extra, Material material)
   {
      if (material != null)
      {
         ...
         for (DAENode tech : techniques)
         {
            ...
            if (profileValue.contains("GOOGLEEEARTH"))
            {
